package com.kobi401.browser.security.adblock;
...
public class AdBlocker {
    ...
    public AdBlocker(LinkWebBridge linkWebBridge) {
        if (linkWebBridge == null) {
            logger.severe("JSInjectionSystem is null. AdBlocker will not function.");
            return;
        }
        this.linkWebBridge = linkWebBridge;
        try {
            linkWebBridge.addScript("console.log('AdBlocker Test Script');");
        } catch (Exception e) {
    ...
    public void blockAdsWithJS() {
        ...
        linkWebBridge.addScript(adBlockerScript);
    }
